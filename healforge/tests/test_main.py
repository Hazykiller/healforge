...
def test_repair_attempt_two_receives_verification_feedback(monkeypatch):
    ...
    class RetryAI:
        ...
        def generate_patch(self, context, diagnosis, verification_feedback=""):
...
def test_verify_records_sandbox_result(monkeypatch, tmp_path):
    ...
    def fake_checkout(_url, _sha, root, _pr_number=None):
...
def test_verify_requires_requested_attempt(monkeypatch):
    client = TestClient(main.app)
    main.SESSIONS.clear()
    session_id = "attempt-missing"
    main.SESSIONS[session_id] = {
        "ref": main.RepoRef("test", "repo", 1),
        "pr": {"head": {"sha": "abc123"}},
        "repairs": [{"attempt": 1, "patch": ""}],
        "verifications": [],
    }
    response = client.post("/api/verify", json={"session_id": session_id, "attempt": 2})
    assert response.status_code == 400
